...
function SortableFeature({
  ...
  return (
    <article ref={setNodeRef} style={style} className={`bot-row feature-row ${isDragging ? 'is-dragging' : ''}`}>
      <button className="drag-handle" type="button" aria-label={`Move ${feature.label}`} {...attributes} {...listeners}>
        <span>⋮⋮</span>
      </button>
      <div className="bot-order">{index + 1}</div>
      <div className="bot-copy">
      ...
      <button
        ...
        aria-label={feature.required ? `${feature.label} is required` : `Remove ${feature.label}`}
      >
        {feature.required ? 'Required' : 'Remove'}
      </button>
    </article>
  );
}
  ...
  return (
    <main className="login-shell">
      <section className="login-card">
        ...
        <p className="eyebrow">DOMAIN SITE ACCESS</p>
        <h1>Site Manager</h1>
...
export default function App() {
  ...
  const catalogById = useMemo(
    () => new Map(catalog.map(feature => [feature.id, feature])),
    [catalog]
  );

  const visibleFeatures = useMemo(
    () => navigation.map(id => catalogById.get(id)).filter((feature): feature is NavigationFeature => Boolean(feature)),
    [catalogById, navigation]
  );

  const hiddenFeatures = useMemo(
    () => catalog.filter(feature => !navigation.includes(feature.id)),
    [catalog, navigation]
  );
  ...
  useEffect(() => {
    if (!authenticated || !selectedSite) return;
    if (mode === 'bots') void loadBots(selectedSite);
    else void loadSettings(selectedSite);
  }, [authenticated, selectedSite, mode, loadBots, loadSettings]);
  ...
  const publishBots = async () => {
    ...
    await waitForPublish(created, async () => {
      setBotDirty(false);
  ...
  const publishSite = async () => {
    ...
    const created = await api<PublishResponse>('publish-site', {
      method: 'POST',
      body: JSON.stringify({ site_id: selectedSite, navigation, colors }),
    });
    await waitForPublish(created, async () => {
      setSettingsDirty(false);
      await loadSettings(selectedSite);
    });
  ...
  const publish = async () => {
    ...
    try {
      if (mode === 'bots') await publishBots();
      else await publishSite();
    } catch (err) {
      if (err instanceof ApiError && err.status === 401) setAuthenticated(false);
      else setError(err instanceof Error ? err.message : String(err));
    } finally {
  ...
  const changeMode = (next: ManagerMode) => {
    ...
    if (mode === 'bots') setBotDirty(false);
    ...
    <div className="app-shell">
      <header className="topbar">
        <div className="topbar-brand">
          ...
          <div><strong>Site Manager</strong><small>{selectedDomain?.display_domain || 'Domain access'}</small></div>
        </div>
      ...
      <main className="workspace">
        <section className="hero">
          <p className="eyebrow">DOMAIN SITE CONTROL</p>
          <h1>{selectedDomain ? `Manage ${selectedDomain.display_domain}` : 'Manage domain'}</h1>
          <p>This session is locked to the domain used at login. Choose what you want to update, make the changes locally, then publish through GitHub validation.</p>
        </section>
        ...
        <section className="manager-card">
          <div className="section-head">
            ...
            {currentDirty && <span className="unsaved-pill">UNPUBLISHED CHANGES</span>}
          </div>
        ...
        <section className="manager-card">
          <div className="section-head">
            <div>
              ...
              <div><strong>What do you want to update?</strong><small>Choose one editor at a time</small></div>
            </div>
          </div>
          <select className="domain-select mode-select" value={mode} disabled={publishing} onChange={event => changeMode(event.target.value as ManagerMode)}>
            <option value="bots">Bot Library — add, remove and reorder XML bots</option>
            <option value="site">Navigation & Theme — customize sections, order and colors</option>
          </select>
        </section>
        ...
        {mode === 'bots' ? (
          <section className="manager-card">
            ...
            <div className="bot-list-wrap">
              {loadingBots ? (
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onBotDragEnd}>
          ...
          <section className="manager-card">
            <div className="section-head">
              <div>
                <span className="step">3</span>
                <div><strong>Navigation & theme</strong><small>Only features already available in the template can be added</small></div>
              </div>
              <span className="count-pill">{navigation.length} VISIBLE</span>
            </div>

            {settingsInherited && !settingsLoading && (
              <div className="alert info">This domain is using the template defaults. The first Publish will create its own navigation and color configuration.</div>
            )}

            {settingsLoading || !colors ? (
              <div className="empty-state"><div className="spinner" />Loading site configuration…</div>
            ) : (
              <>
                <div className="settings-subhead">
                  <div><strong>Navigation items</strong><small>Drag to reorder. Remove optional items you do not want.</small></div>
                </div>
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onFeatureDragEnd}>
                  <SortableContext items={navigation.map(featureDndId)} strategy={verticalListSortingStrategy}>
                    <div className="bot-list feature-list">
                      {visibleFeatures.map((feature, index) => (
                        <SortableFeature
                          ...
                          onDelete={() => {
                            ...
                            setNavigation(current => current.filter(id => id !== feature.id));
                            setSettingsDirty(true);
                          }}
                        />
                      ))}
                ...
                <div className="add-feature-row">
                  <select value={featureToAdd} onChange={event => setFeatureToAdd(event.target.value)} disabled={!hiddenFeatures.length}>
                    <option value="">{hiddenFeatures.length ? 'Select a hidden feature to add' : 'All available features are visible'}</option>
                    {hiddenFeatures.map(feature => <option key={feature.id} value={feature.id}>{feature.label}</option>)}
                  </select>
                  <button
                    type="button"
                    ...
                    onClick={() => {
                      if (!featureToAdd || navigation.includes(featureToAdd)) return;
                      setNavigation(current => [...current, featureToAdd]);
                      setFeatureToAdd('');
                      setSettingsDirty(true);
                    }}
                  >
                ...
                <div className="settings-subhead theme-subhead">
                  <div><strong>Navigation colors</strong><small>Current colors are shown below. Pick a color or enter a six-digit hex value.</small></div>
                </div>

                <div className="theme-preview" style={{ background: colors.nav_background, color: colors.nav_text }}>
                  <span style={{ background: colors.primary, color: '#fff' }}>Active item</span>
                  <span style={{ color: colors.secondary }}>◆ Icon accent</span>
                ...
                <div className="color-grid">
                  {COLOR_FIELDS.map(field => (
                    <label className="color-field" key={field.key}>
                      <div><strong>{field.label}</strong><small>{field.help}</small></div>
                      <div className="color-inputs">
                        <input
                          type="color"
                          value={colors[field.key]}
                          onChange={event => {
                            setColors(current => current ? { ...current, [field.key]: event.target.value.toLowerCase() } : current);
                            setSettingsDirty(true);
                          }}
                        />
                        <input
        ...
        <section className="publish-card">
          <div>
            ...
            <div>
              <strong>Publish {mode === 'bots' ? 'bot library' : 'navigation & theme'}</strong>
              <small>Branch → PR → Node 22/24 checks → merge → Netlify</small>
            </div>
          </div>
          <button className="primary-button publish-button" type="button" disabled={!selectedSite || busy} onClick={publish}>

...
export function MyWebsitesView({
  ...
  return <div className="websites-page">
    <section className="websites-heading"><div><p>OWNED WEBSITES</p><h2>My Websites</h2><span>Build the website first. Domains, deployment and payment remain separate stages.</span></div><button className="v2-primary-button" type="button" onClick={onCreateWebsite}>+ Create website</button></section>
    ...
    {!websites.length ? <section className="websites-empty"><div className="websites-empty-icon">＋</div><p>NO WEBSITES YET</p><h3>Start with nothing. Build your first website here.</h3><span>You do not need a domain or payment first. Create the website, configure the real nnn template, preview it, then prepare hosting.</span><button className="v2-primary-button" type="button" onClick={onCreateWebsite}>Create your first website</button></section> : <section className="website-card-grid">
...
export function CreateWebsiteView({ onCreated, onCancel }: { onCreated: (website: WebsiteRecord) => void; onCancel: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try { const result = await websiteRequest('', { method: 'POST', body: JSON.stringify({ name }) }); if (!result.website) throw new Error('The website record was not returned.'); onCreated(result.website); }
    catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setBusy(false); }
  };
  return <div className="website-create-page"><section className="website-create-card"><p className="v2-kicker">CREATE WEBSITE · START</p><h2>Give your website a working name.</h2><p>No domain or payment is required. Site Manager creates a private website owned by your account and opens the nnn template builder.</p><form onSubmit={submit}><label>Website name<input value={name} onChange={event => setName(event.target.value)} required minLength={2} maxLength={100} placeholder="Example: Duke Trading Hub" autoFocus /></label><div className="website-create-preview"><span>Template</span><strong>nnn</strong><span>Starting status</span><strong>Draft</strong><span>Domain required now?</span><strong>No</strong><span>Payment required now?</span><strong>No — designed later</strong></div>{error && <div className="websites-alert error">{error}</div>}<div className="website-create-actions"><button type="button" onClick={onCancel}>Cancel</button><button className="v2-primary-button" type="submit" disabled={busy}>{busy ? 'Creating…' : 'Create & configure website'}</button></div></form></section></div>;
}
